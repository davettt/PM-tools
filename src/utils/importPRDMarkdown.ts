...
function parseGoals(text: string): {
  ...
  const successMetrics: PRDSuccessMetric[] = parseBulletList(metricsMatch?.[1]?.trim() ?? '').map(
    m => ({ id: uuidv4(), metric: m })
  );
...
function parseScenarios(text: string): PRDScenario[] {
  ...
  for (const part of parts.slice(1)) {
    ...
    const content = newline === -1 ? '' : stripPlaceholder(part.slice(newline + 1).trim());
...
export function parsePRDMarkdown(markdown: string): Partial<PRDForm> {
  ...
  const requirements: PRDRequirementItem[] = parseRequirementsList(sections['Requirements'] ?? '');

  const outOfScope: PRDOutOfScopeItem[] = parseBulletList(sections['Out of Scope'] ?? '').map(
    d => ({ id: uuidv4(), description: d })
  );

  const openQuestions: PRDOpenQuestion[] = parseBulletList(sections['Open Questions'] ?? '').map(
    q => ({ id: uuidv4(), question: q })
  );

  const timelineText = sections['Timeline'] ?? '';
  const timeline = timelineText.includes('|') ? parseTimelineTable(timelineText) : [];
