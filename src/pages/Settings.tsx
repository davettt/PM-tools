...
const Settings = () => {
  ...
  const [prdImportResult, setPrdImportResult] = useState<ImportResult | null>(null);
  const [reviewImportResult, setReviewImportResult] = useState<ImportResult | null>(null);
  ...
  const handleExportPRDs = () => downloadJSON(activePRDs, `pm-tools-prds-${dateStamp()}.json`);
  ...
  const handleImportFile = async (
    ...
    try {
      ...
      setImportError('Could not import file. Make sure it is a valid PM Tools backup JSON.');
    }
  };

  const handlePRDImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) handleImportFile(file, '/api/prds/import', setPrdImportResult, fetchPRDs);
    e.target.value = '';
  };

  const handleReviewImport = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) handleImportFile(file, '/api/reviews/import', setReviewImportResult, fetchReviews);
    e.target.value = '';
  };
      ...
      <main className="max-w-2xl mx-auto px-6 py-8 space-y-10">
        <section>
          ...
          <p className="text-sm text-gray-500 mb-6">
            Export downloads a JSON backup of all active documents. Import merges a backup into your
            current data — new documents are added, newer versions replace older ones, and nothing
            is deleted.
          </p>
          ...
          <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {activePRDs.length} {activePRDs.length === 1 ? 'document' : 'documents'}
                  </p>
                </div>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium text-gray-900 text-sm">Acceptance Reviews</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    {activeReviews.length} {activeReviews.length === 1 ? 'document' : 'documents'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <button
