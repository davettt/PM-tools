...
const PRD = () => {
  ...
  const [enhanceResult, setEnhanceResult] = useState<PRDEnhancementResult | null>(null);
  ...
  const applyPRDEnhancements = (accepted: PRDAcceptedChanges) => {
    ...
    // Text fields
    if (accepted.sections.overview !== undefined) patch.overview = accepted.sections.overview;
    if (accepted.sections.problemStatement !== undefined)
      patch.problemStatement = accepted.sections.problemStatement;
    if (accepted.sections.objective !== undefined) patch.objective = accepted.sections.objective;
    ...
    if (Object.keys(accepted.requirements).length > 0) {
      patch.requirements = form.requirements.map(r => {
        ...
        const updated = improved !== undefined ? { ...r, description: improved } : r;
  ...
  const addScenario = () => {
    ...
    setTimeout(() => {
      const inputs =
        scenariosSectionRef.current?.querySelectorAll<HTMLInputElement>('input[type="text"]');
      inputs?.[inputs.length - 1]?.focus();
    }, 0);
  };
  const updateScenario = (id: string, patch: Partial<PRDScenario>) =>
    update({
      scenarios: form.scenarios.map(s => (s.id === id ? { ...s, ...patch } : s)),
    });
    ...
    update({
      requirements: form.requirements.map(r =>
        r.id === reqId
          ? {
              ...r,
              subtasks: [...(r.subtasks ?? []), { id: uuidv4(), description: '' }],
            }
          : r
      ),
    });
  ...
  const updateSubtask = (reqId: string, subId: string, patch: Partial<PRDSubtaskItem>) =>
    update({
      requirements: form.requirements.map(r =>
        r.id === reqId
          ? {
  ...
  const updateMetric = (id: string, patch: Partial<PRDSuccessMetric>) =>
    update({
      successMetrics: form.successMetrics.map(m => (m.id === id ? { ...m, ...patch } : m)),
    });
  ...
  const removePhase = (id: string) => update({ timeline: form.timeline.filter(p => p.id !== id) });
            ...
            {(
              ...
              <div key={field}>
                ...
                <input
                  ...
                  onChange={e => update({ meta: { ...form.meta, [field]: e.target.value } })}
          ...
          <div className="mt-3 grid grid-cols-1 gap-3">
            ...
            <div>
              ...
              <input
                ...
                onChange={e => update({ meta: { ...form.meta, docLink: e.target.value } })}
        ...
        {(() => {
          ...
          const pairs: [string, string, string, string][] = (
            [
              ...
              ['Version', form.meta.version, 'Target Launch', form.meta.targetLaunch],
              ['Product Area', form.meta.productArea, 'Dev Lead', form.meta.engineeringLead],
            ...
            {form.scenarios.map(scenario => (
              ...
              >
                ...
                <textarea
                  ...
                  onChange={e => updateScenario(scenario.id, { content: e.target.value })}
        ...
        <section>
          ...
          <div className="space-y-2" ref={outOfScopeSectionRef}>
            {form.outOfScope.map(item => (
              <SectionRow key={item.id} onRemove={() => removeOutOfScope(item.id)}>
                <textarea
                  rows={1}
                  value={item.description}
                  onChange={e => updateOutOfScope(item.id, { description: e.target.value })}
                  onKeyDown={e => {
                    if (e.key === 'Enter' && !e.shiftKey) {
                      e.preventDefault();
                      addOutOfScope(true);
              ...
              >
                ...
                <div>
                  ...
                  <input
                    type="text"
                    value={phase.dates}
                    onChange={e => updatePhase(phase.id, { dates: e.target.value })}
